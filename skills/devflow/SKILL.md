...
| Commit, publish or update a PR/stack | [Implementing](../devflow-implementing/SKILL.md) | `devflow-implementer` |
| Merge a PR or stack | [Merging](../devflow-merging/SKILL.md) | the session itself |
...
"Use devflow to fix the retry bug in this repository" is a defined implementation request: coordinating claims the issue, dispatches the implementer, opens the PR after the first meaningful commit, verifies the fix and merges only when requested. Sequential unmerged features join the same gh stack, even without a functional dependency; see [PR workflow](references/pr-workflow.md). "How does the retry code work?" is a question: answer it without creating work records, issues or agents.
