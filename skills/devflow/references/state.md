...
Invoke this helper from the coordinating session while following a Devflow skill; delegated agents return their results and the coordinator records them, because their sandboxes cannot write the database. Resolve `scripts/state.py` relative to the installed `devflow` skill directory. For the default installation:
...
Hooks store identifiers, counts, timestamps and command fingerprints, not prompts, commands or tool-output text. They never call a model. The only tool decision they make is the coordinator boundary: for a session bound as coordinator, a `PreToolUse` that would edit files, run a mutating Git command, edit in place or redirect output outside temporary paths is denied and recorded as a `boundary` event; read-only forms such as `git tag --list`, `git stash list`, dry runs and `git apply --check` pass. Git flags are scoped to their own shell command and stop at `--`; `git apply --apply` is mutating even alongside inspection flags. Coordinator `gh pr merge` and `gh stack merge` calls remain available for the authorized final merge; agent sessions are never blocked, and the decision is made before transcript collection so a collector failure cannot let the call through. It is a backstop for the coordinating skill, not a sandbox: changes made through interpreters or file copies are not detected. Transcript parsing is an adapter for the current local format; missing counters or a reset remain visible gaps, and a malformed line is recorded as a `transcript_gap` event and skipped so one bad line cannot stop collection. The next counter after a skipped line re-establishes the baseline without allocating usage and is recorded as a `counter_baseline` gap; usage hidden by the skipped line stays unknown rather than being charged to the work. Replay and partial transcript lines do not add duplicate usage. Trust installed hooks using `/hooks`; untrusted hooks do not collect anything. A fresh task may be needed after installing them.
