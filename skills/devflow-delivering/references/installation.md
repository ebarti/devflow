...
Use the immutable source revision selected by the reviewed release. Prepare `install plan` with explicit managed skill/launcher/config targets, known shared consumers and preserved files. Review concrete before/after changes within existing user authority; do not infer enrollment of another repository.

For 0.5+, install every stage from `skill list` alongside the entry and compatibility alias. A partial catalog is incomplete cutover. Update the small global instruction to require using-devflow before response/action and when intent changes; loading it performs no backlog scan, claim or launch.
