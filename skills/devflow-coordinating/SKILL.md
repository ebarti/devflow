...
4. **Choose the agent.** Reuse an existing agent when the next implementation is the same or closely related to what it already did, as the [worker reference](../devflow/references/implementation-worker.md#reuse) allows; otherwise spawn the `devflow-implementer` agent by type with the arguments defined there. Never pass a model or effort, use another role for implementation, or let a worker inherit the coordinator's model. On a transient spawn failure, wait or reuse a suitable worker as the reference describes; on a configuration failure, stop and ask the user. Never implement directly. Coordinator, review and verification agents remain unchanged.
