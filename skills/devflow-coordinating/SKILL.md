...
3. **Sharpen before dispatching.** Before spawning, hold the accepted outcome, the slice with its owned files and dependencies, and exact checks or manual steps with expected results, from [defining work](../devflow-defining-work/SKILL.md) and [planning](../devflow-planning/SKILL.md) as the request required. If you cannot write them down, sharpen the plan instead of delegating the ambiguity.

4. **Choose the agent.** Every delegated action runs as its [Devflow agent type](../devflow/references/agents.md): `devflow-implementer` for implementation, `devflow-reviewer`, `devflow-verifier` and `devflow-deliverer` for review, verification and delivery, and `devflow-definer` or `devflow-planner` when you delegate definition or planning instead of doing it yourself. Reuse an existing agent when the next task is the same or closely related work in the same role, work ID and worktree, as the [worker reference](../devflow/references/implementation-worker.md#reuse) describes; otherwise spawn by type with `fork_turns: "none"`. Never pass a model or effort, use a generic role, or let a worker inherit the coordinator's model. On a transient spawn failure, wait or reuse a suitable agent; on a configuration failure, stop and ask the user. Never implement directly.
