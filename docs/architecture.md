...
| Host workflow skill | One `devflow` skill with targeted intake, subagent, implementation, review, QA and delivery references | Engineering behavior and the bridge to supported Codex subagent tools | Duplicate state machine, token arithmetic or a second backlog |
...
5. **Verify independently.** Freeze a clean candidate. For each required gate, the coordinator activates a verified review or QA subagent with the same candidate contract. Review inspects correctness/contracts; QA executes the affected product path. Each returns a gate result and structured findings tied to exact evidence. Their identities are distinct from the coordinator, implementation worker and each other. They do not silently repair the product while judging it. Tier 0 requires static checks and skips independent gates.
