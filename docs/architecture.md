...
A newly requested continuation of a verified completed PR reactivates that same work through `work reopen`. Fresh request authority and an immutable continuation link the original delivery to the new execution period, with transactional claim reacquisition. The original attempt ID, entry phase, delivery and producer evidence remain historical facts. Repair starts at Implement; delivery-only entry can reuse proof only when its candidate and operative inputs remain valid. This bounded transition retains the same PR endpoint and does not enable automatic merge or transfer its coordinator.
...
The 0.5 subagent snapshot contract prevents repair while an activated independent result is unimported. Each gate binds its assignment activation; original mismatched results remain retrievable history. A completed producer can correct rejected evidence serialization through a journaled continuation that preserves its original judgement, artifact and activation; this does not rerun product work. Publication includes a journaled branch push with expected source and remote head. Real deferred findings require linked work, and delivery records actual accounting completeness or an explicit unavailable/unknown limit. These guards apply at the CLI boundary; instruction hashes prove captured bytes, not model compliance or shell interception.
