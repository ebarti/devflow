...
This page records the reusable package boundary through the 0.5.2 completed-PR continuation change, with the initial 2026-09-10 rollout baseline retained below. Version 0.5.1 clarified retained artifact retrieval and request-file syntax. Version 0.5.2 adds explicit same-PR continuation with fresh authority, retained history and transactional claim reacquisition. The [implementation plan](implementation-plan.md) remains the target for the complete rollout. Package publication and repository adoption are separate steps; package tests alone do not establish JobCtrl cutover.
...
| Installation and skill | Clean committed source archive, immutable revision/content check, independent scope/digest, journaled apply/rollback, legacy routing function, complete discoverable stage skills with pinned resolution; actual interrupted installer tests | Each shared-host update needs a concrete consumer inventory and authorized manifest; consumer pins and host readback are separately verified |
...
The stage architecture, action-owner routing, gate-ingestion ordering, historical result retention, guarded branch publication, inline-anchor compatibility and explicit delivery accounting/deferrals have focused package tests. Behavioral forward tests, installed catalog readback and an ordinary adopter run are recorded against the release candidate separately. A green package suite is not a claim that the native host or live GitHub path passed. Failures in the ordinary run remain evidence even after successful resumption.

A25 adds real CLI and durable-state proof for continuing a verified completed PR, including rejection of insufficient authority, changed identities, unfinished operations and stale proof. The actual resumed adopter PR and a fresh uninterrupted ordinary backlog run remain distinct operational evidence. This transition does not establish protected automatic merge conformance.
