...
Use the [stabilization and deferred comparison plan](/Users/eloibarti/Documents/Codex/2026-09-09/jobctrl-workflow-assessment/comparison-plan.md): stabilize the full workflow using the user's defaults, then run paired independent implementations of the same cases under two configurations with a fixed workflow and explicit duplicate-work budget. Retain unsuccessful attempts, publish sample sizes, and distinguish preliminary screening from evidence sufficient to change defaults.
