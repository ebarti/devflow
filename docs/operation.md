...
`work prepare` reports missing contract fields and execution-source prerequisites without creating state or inventing authority. Its `missing` list identifies `source.lineage` when absent and `source.consumed_digest` when missing or different from canonical `digest(lineage)`. The agent follows the [capture-to-source recipe](../skills/devflow-defining-work/references/source-lineage.md), preserving unknown origin and all consumed observations; the issue content digest belongs inside its observation and is not the aggregate digest. Even `missing: []` retains `ready: false` and `authority_required: true`. Legacy source records remain schema-readable. `work ready` requires a contract and `user_request` with `reference`, `summary`, and `allowed_operations`; it derives and returns `admission_id`, `authority_id`, and `scope_hash`. `work amend` accepts the same request shape for an authorized scope revision without a separate `approved_delta` field. The agent supplies these fields from the conversation. Caller Authority records, labels and invented queue-policy references cannot substitute for a request. `snapshot capture` stores the actual used instruction bytes, profile, full package revision, and observed nonsecret settings. `work start` verifies the snapshot's executing revision, admitted profile and stored inputs before claiming an attempt and returning a workspace intent. A missing or mismatched pin is rejected before creating an active claim.
...
Classify an explicitly requested completed-PR upgrade before ordinary doctor preflight when its owned checkout retains an older pin. Use the reviewed installed release's reader to inspect saved work/next and the actual PR, then its explicit snapshot capture and same-work reopen procedure. Pre-admission doctor BLOCKED for that historical pin is expected. After admission, require that release's `doctor --work-id <existing-id> --repository <owned-checkout> --state-dir <existing-store> --json` to return READY before role activation: the now-active work selects the newly admitted snapshot. Without the work ID, generic doctor remains BLOCKED while the actual lock is old. Missing tools, invalid profile/release, unresolved operations and failed capture/admission remain blockers; this route never executes the old release or edits its pin before authority. Other enrolled work retains ordinary doctor preflight.
