...
Role launch intents are executed by the active owner through supported native task tools. `host prepare` includes `work_id` and the current `expected_revision`, and accepts only the current active attempt's prepared action and recorded assignment. Canceled work, invalidated/failed actions, stale scopes/candidates and stale revisions cannot produce a launch intent. Dispatched or uncertain native operations expose recovery only. Record pending setup separately from the final task ID. A task title alone cannot reconcile a lost creation response. Use the assignment marker and observed prompt/context. Review and QA return structured results from their registered independent identities; the owner does not supply a substitute PASS.
