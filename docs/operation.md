...
Open the same repository and configured private state root. Run `backlog list` to discover saved intake requests and `work list` to discover admitted work. Resume intake with `backlog capture --work-id <id> --json`; the saved title/body and dispatch state are sufficient. Resume an attempt with `work show` and `next`, retaining its captured package revision, checkout, evidence and role identities. No original request file or owner conversation is needed for these recorded facts.
